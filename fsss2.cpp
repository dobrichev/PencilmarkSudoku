...
single_found:
...
#endif
	//if(guessDepth < 1)
	//if(guessDepth == 0 || (guessDepth < 8 && (numTrialsLocal > 1000))) // works wery well!
	//if(guessDepth == 0 || (guessDepth < 14 && (numTrialsLocal > 400))) // works well for some/most hardest (200" -> 6"; 180" -> 4")
	if(guessDepth == 0 || numTrialsLocal > 400) // works well for some/most hardest (200" -> 4"; 180" -> 3")
	{
		bool found = false;
...
#ifdef USE_SUBSETS
	//if(solved.popcount_128() < 3) {
	//if(guessDepth < 1) {
	//if(guessDepth == 0 || guessDepth < 3 && (numTrialsLocal > 500 && maxGuessDepth > 28)) {
	if((guessDepth == 0 && solved.popcount_128() < 3) || (guessDepth < 6 && (numTrialsLocal > 200))) { // works very good!
		int eliminationFound = false;
	//	subsetsDone = 1;
		eliminationFound = doPairs();
		if(eliminationFound) {
			if(eliminationFound < 0) goto contradiction;
			goto nakedAgain;
		}
		eliminationFound = doSeptuples();
		if(eliminationFound) {
			if(eliminationFound < 0) goto contradiction;
			goto nakedAgain;
		}
	...
	//if(subsetsDone == 1) {
	//	subsetsDone = 2;
		eliminationFound = doTriples();
		if(eliminationFound) {
			if(eliminationFound < 0) goto contradiction;
			goto nakedAgain;
		}
	//}
		eliminationFound = doSextuples();
		if(eliminationFound) {
			if(eliminationFound < 0) goto contradiction;
			goto nakedAgain;
		}
...
#ifdef USE_FISHES
	//if(guessDepth < 19 && ((numTrialsLocal > 3000) || maxGuessDepth > 25) && solved.popcount_128() < 43)
	if(guessDepth < 20 && ((numTrialsLocal > 3000) || maxGuessDepth > 25) && solved.popcount_128() < 20)
	//if(maxGuessDepth > 25)
	//if(solved.popcount_128() < 10) // 310" tom's hardest doFish2; 368" w/o it
	{
		int eliminationFound = false;
//		eliminationFound = doFish2();
//		if(eliminationFound) {
//			//if(eliminationFound < 0) goto contradiction;
//			goto nakedAgain;
//		}
		eliminationFound = doFish3();
		if(eliminationFound) {
			//if(eliminationFound < 0) goto contradiction;
			goto nakedAgain;
		}
...
//			}
...

		{
			...
			//later continue with this candidate eliminated	//bm128 knownNoQuintuples[126]; // almost never helps

			if(guessAuto)gg[optDigit].clearBit(optCell); //unless the guess choice has been enforced by the collector
		}
		//if(maxGuessDepth < guessDepth) maxGuessDepth = guessDepth;
		//try the "optimal" cell/digit candidate
		//if(optCell < 0 || optCell > 80 || optDigit < 0 || optDigit > 8) printf("\nBug: optCell=%d, optDigit=%d\n", optCell, optDigit);
...
template < class X > void fsss2 < X > ::solve(const pencilmarks& forbidden) {
	...
	doEliminations();
	//if(numTrialsLocal > 1000) fprintf(stderr, "maxGuessDepth=%d\tnumTrialsLocal=%d\n", maxGuessDepth, numTrialsLocal);
}
...
bool isRedundant::isAllowedRedundant(const pencilmarks& forbiddenValuePositions, int testValue, int testPosition) {
	fsss2<isRedundant> solver(*this);
	nsol = 0;
	solver.initEmpty(); //all values are allowed everywhere
	for(int g = 0; g < 9; g++) {
	...
	solver.grid[testValue].clearBit(testPosition); //disallow testValue in testPosition
	solver.doEliminations();
	return (nsol == 0); //if no solution exist, then forbidding the testValue is redundant
}
